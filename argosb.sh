...
sed -i '/yonggekkk/d' ~/.bashrc
echo "export nix=y uuid=${uuid} vmpt=${port_vm_ws} agn=${ARGO_DOMAIN} agk=${ARGO_AUTH} && bash <(curl -Ls https://raw.githubusercontent.com/hkjp123/argosb/main/argosb.sh)" >> ~/.bashrc
source ~/.bashrc
fi
sleep 2
